...
NAME = ["name"]
GW_COL = ["GW"]
FEATURES = ["xP", "team", "position", "bps", "minutes", "ict_index", "yellow_cards", "red_cards", "selected", "starts","transfers_in", "transfers_out", "value"]
FUTURE_FEATURES = ["opponent_team", "was_home", "total_points"]
FUTURE_FEATURES_RENAME = ['opponent_team_next', 'is_home_next']
    ...
    for player in players:
        ...
        for gw in PAST_GWS: 
            if gw + 1 < CURRENT_GW: # can only get future data from the week before NOW and earlier
                ...
                next_gw = next_gw.rename(
                    columns={"total_points": "total_points_next", "opponent_team": "opponent_team_next", "was_home": "is_home_next"}
                )
...
def predict(model, players: list[Player], df: pd.DataFrame):
    for player in players:
        ...
        future_data = pd.DataFrame(
            {
                "name": [player.name],
                "opponent_team_next": [player.get_vs_gw(CURRENT_GW).id],
